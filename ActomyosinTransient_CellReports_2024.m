...
% Explicit scheme
% Unknowns:
% pc, pb_N, pf_1, vc, vb, vf, thetan, vn, thetac, mn, mc, cc, cb, cf
...
%===============================
% gamma = gamma0 + gammaa*(ksigmaa*mn).^2;
%===============================
...
dx_ref = 5;             % (micron) reference grid size 
...
Mn = 5d-3;              % (mM) reference value of activated  myosin
Mt = Mc + Mn;           % (mM) reference total myosin

%% Cell dynamics parameters
ksigman = 8d3;          % (Pa /mM) Coefficient of passive actin network pressure
ksigmaa = 2d4;          % (Pa /mM) Coefficient of active actin network contraction
gamma0 = 3.0d-3;        % (1/s) constant rate of actin depolymerization
gammaa = 1.0d-5;        % (1/Pa/s)
Jactinf0 = 3.5d-3;      % (micron mM/s) Jactinf = Jactinf0*thetac^f/(thetacc + thetac^f)
...
kon  = 0.4d-0;          % (1/s mM) kon*m_c*theta_n
koff = 1.1d-0;          % (1/s) koff*m_n 
...
Dmc = 1.d0;             % (micron^2/s) diffusion constant for m_c
Dmn = 1.d-1;            % (micron^2/s) diffusion constant for m_n
...
MyoRatio = NaN(Nt,1);   % Myosin ratio from upstream to downstream
ActInt = NaN(Nt,1);
ActCon = NaN(Nt,1);
MyoInt = NaN(Nt,1);
MyoCon = NaN(Nt,1);
...
mc = Mc*ones(Nc,1);
mn = Mn*ones(Nc,1);
sigma = ksigman*thetan - ksigmaa*mn;
...
for it = 2:Nt
    ...
    mn = GridUpdate_con2(mn,xb(it-1),xf(it-1),xb(it),xf(it),dx_ref,vb(it),vf(it),dt);
    mc = GridUpdate_con2(mc,xb(it-1),xf(it-1),xb(it),xf(it),dx_ref,vb(it),vf(it),dt);
    ...
    % update ghost points for mn to calculate thetan
    mn_ghost_b = mn(2)    + 2*dxc/Dmn*(vb(it)-vn(1))*mn(1);
    mn_ghost_f = mn(Nc-1) + 2*dxc/Dmn*(vn(end)-vf(it))*mn(Nc);
    mn_temp = [mn_ghost_b; mn; mn_ghost_f];
    ...
    thetan_ghost_b = 2/ksigman*((eta_it+eta_st)*vb(it)*dxc*thetan(1) ...
        ...
        - ksigmaa/2*(mn_temp(3)-mn_temp(1)) + (eta_it+eta_st)*dxc*Jactinb);
    thetan_ghost_f = -2/ksigman*((eta_it+eta_st)*vf(it)*dxc*thetan(Nc) ...
        - eta_it*vc(it)*dxc*thetan(Nc) - ksigman/2*thetan(Nc-1) ...
        - ksigmaa/2*(mn_temp(Nc+2)-mn_temp(Nc))  - (eta_it+eta_st)*dxc*Jactinf);
    ...
    thetan = thetan + dt*(ksigman/(eta_it+eta_st)/dxc^2 ...
        *(thetan_temp(3:Nc+2)-2*thetan_temp(2:Nc+1)+thetan_temp(1:Nc)) ...
        ...
        - ksigmaa/(eta_it+eta_st)/dxc^2*(mn_temp(3:Nc+2)-2*mn_temp(2:Nc+1)+mn_temp(1:Nc)) ...
        - gamma.*thetan);
    ...
    vn = eta_it/(eta_it+eta_st)*vc(it) - 1./thetan*ksigman/(eta_it+eta_st)...
        ...
        + 1./thetan*ksigmaa/(eta_it+eta_st).*(mn_temp(3:Nc+2)-mn_temp(1:Nc))/2/dxc;
    
    %% Myosin module
    % ghost points for mn
    mn_ghost_b = mn(2)    + 2*dxc/Dmn*(vb(it)-vn(1))*mn(1);
    mn_ghost_f = mn(Nc-1) + 2*dxc/Dmn*(vn(Nc)-vf(it))*mn(Nc);
    mn_temp = [mn_ghost_b; mn; mn_ghost_f];
    % ghost points for mc
    mc_ghost_b = mc(2)    + 2*dxc/Dmc*(vb(it)-vc(it))*mc(1);
    mc_ghost_f = mc(Nc-1) + 2*dxc/Dmc*(vc(it)-vf(it))*mc(Nc);
    mc_temp = [mc_ghost_b; mc; mc_ghost_f];

    Mat = zeros(2*Nc, 2*Nc);
    RHS = zeros(2*Nc, 1);

    % Matrix and RHS for mn and mc at i = 1 and i = Nc
    Mat(1,1)     = - (2*Dmn/dxc^2 - vn(1)/dxc + 1/dt + koff);
    Mat(1,2)     = Dmn/dxc^2 - vn(2)/dxc;
    Mat(1,Nc+1)  = kon*thetan(1);
    Mat(Nc,Nc-1) = Dmn/dxc^2 + vn(Nc-1)/dxc;
    Mat(Nc,Nc)   = - (2*Dmn/dxc^2 + vn(Nc)/dxc + 1/dt + koff);
    Mat(Nc,2*Nc) = kon*thetan(Nc);
    
    Mat(Nc+1,1)      = koff;
    Mat(Nc+1,Nc+1)   = - (2*Dmc/dxc^2 - vc(it)/dxc + 1/dt + kon*thetan(1));
    Mat(Nc+1,Nc+2)   = Dmc/dxc^2 - vc(it)/dxc;
    Mat(2*Nc,Nc)     = koff;
    Mat(2*Nc,2*Nc-1) = Dmc/dxc^2 + vc(it)/dxc;
    Mat(2*Nc,2*Nc)   = - (2*Dmc/dxc^2 + vc(it)/dxc + 1/dt + kon*thetan(Nc));

    RHS(1)    = - Dmn/dxc^2*mn_ghost_b - mn(1)/dt;
    RHS(Nc)   = - Dmn/dxc^2*mn_ghost_f - mn(Nc)/dt;
    RHS(Nc+1) = - Dmc/dxc^2*mc_ghost_b - mc(1)/dt;
    RHS(2*Nc) = - Dmc/dxc^2*mc_ghost_f - mc(Nc)/dt;

    % Matrix and RHS for mn and mc from i = 2 to i = Nc-1
    for in = 2:Nc-1
        Mat(in,in-1)  = Dmn/dxc^2 + vn(in-1)/2/dxc;
        Mat(in,in)    = - (2*Dmn/dxc^2 + koff + 1/dt);
        Mat(in,in+1)  = Dmn/dxc^2 - vn(in+1)/2/dxc;
        Mat(in,Nc+in) = kon*thetan(in);

        Mat(Nc+in,in) = koff;
        Mat(Nc+in,Nc+in-1) = Dmc/dxc^2 + vc(it)/2/dxc;
        Mat(Nc+in,Nc+in)   = - (2*Dmc/dxc^2 + kon*thetan(in) + 1/dt);
        Mat(Nc+in,Nc+in+1) = Dmc/dxc^2 - vc(it)/2/dxc;
    ...
    MyoRatio(it) = mn(1)/mn(Nc);
    sigma = ksigman*thetan - ksigmaa*mn;
    
    ActInt(it) = sum(thetan(1:Nc-1)+thetan(2:Nc))*dxc/2 ...
        + sum(thetac(1:Nc-1)+thetac(2:Nc))*dxc/2;
    ActCon(it) = ActInt(it)/(Thetat*Lc(1));
    MyoInt(it) = sum(mn(1:Nc-1)+mn(2:Nc))*dxc/2 + sum(mc(1:Nc-1)+mc(2:Nc))*dxc/2;
    MyoCon(it) = MyoInt(it)/(Mt*Lc(1));
    ...
    if mod((it-1),round(dt_plot/dt)) == 0
        ...
        figure(2) 
        subplot(4,1,1) % G-actin concentration thetac
        ...
        subplot(4,1,2) % F-actin concentration thetan
        ...
        subplot(4,1,3) % mc concentration
        plot(xc ,mc*1d3,'k','linewidth',1); hold off
        xlim([xb(it),xf(it)]);
        set(gca,'fontsize',13)
        xlabel('x (µm)','fontsize',13)
        ylabel('m_c (µM)','fontsize',13) 
        
        subplot(4,1,4) % mn concentration
        plot(xc ,mn*1d3,'k','linewidth',1); hold off
        xlim([xb(it),xf(it)]);
        set(gca,'fontsize',13)
        xlabel('x (µm)','fontsize',13)
        ylabel('m_n (µM)','fontsize',13) 
        set(gcf,'color','w');
...
%         plot([0,L],[w,w],'-k','linewidth',1); 
%         rectangle('Position',[xb(it) 0 Lc(it) w],'Curvature',w/1d2); 
%         [~,hd] = contour(Xmesh,Ymesh,mn_plot,'fill','on'); colorbar; hold off
%         set(hd,'LevelList',linspace(min(min((mn_plot))),max(max((mn_plot))),25));
%         xlabel('Length (µm)')
%         ylabel('Width (µm)')
%         % set(gca,'xtick',[],'ytick',[])
%         title(['t =  ',num2str((it-1)*dt/60),' min (myosin concentration in µM)'],'FontWeight','Normal');
%         set(gca,'fontsize',11)
%         box off
%         %axis equal
% 
...
figure(11)
subplot(2,1,1)
plot(Time/60,(vb+vf)/2*3600,'-','linewidth',1); 
set(gca,'fontsize',13)
xlim([0 T_total/60])
xlabel('time (min)','fontsize',13)
...
box off
subplot(2,1,2)
plot(Time/60,ActRatio,'-','linewidth',1); hold on
plot(Time/60,MyoRatio,'-','linewidth',1); hold off
set(gca,'fontsize',13)
xlim([0 T_total/60])
xlabel('time (min)','fontsize',13)
...
figure(13)
subplot(3,1,1) % Actin Conservation factor
...
subplot(3,1,2) % Myosin Conservation factor
plot(Time/60,MyoCon,'-k','linewidth',1);
set(gca,'fontsize',13)
xlim([0 T_total/60])
xlabel('time (min)','fontsize',13)
ylabel('Myosin Con.','fontsize',13)

subplot(3,1,3) % Cell length ratio
